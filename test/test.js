...
describe("colorsys", function() {
  ...
  it("should convert hex <-> rgb", function() {
    ...
    expect(colorsys.hex_to_rgb("#66d7a9")).to.deep.equal({
      r: 102,
      g: 215,
      b: 169
    })
  ...
  it("should retrieve the correct stringEnconding", function() {
    ...
    expect(colorsys.getColorEncoding({ h: 150, s: 0.2, v: 0.5 })).to.equal(
      "hsv"
    )
    expect(colorsys.getColorEncoding({ h: 150, s: 0.2, l: 0.5 })).to.equal(
      "hsl"
    )
    expect(colorsys.getColorEncoding({ j: 150, f: 0.2, r: 0.5 })).to.equal(
      "unknown"
    )
    expect(
      colorsys.getColorEncoding({ c: 150, m: 0.2, y: 0.5, k: 4 })
    ).to.equal("cmyk")
  })
})
